...
export function apply(ctx, config = {}) {
  ...
  function launchBot(bot, appSecret) {
    ...
    return startInboundForBot({
      ...
      helpers: { log, lookupReplyMapping, readBotState, writeBotState },
    }).catch((err) => {
...
async function startInboundForBot(opts) {
  ...
  function accept(event) {
    ...
    // ★ 消息级路由：优先按被回复消息(parent_id/root_id)查映射；未命中走聊天级固定会话
    const repliedTo = event.message.parent_id ?? event.message.root_id ?? null
    const mapped = helpers.lookupReplyMapping(repliedTo)
  ...
  async function handle(event, mapped) {
    ...
    try {
      ...
      await replyToMessage(messageId, answer || '（该回合结束但没有文本回复）')
    } catch (err) {
      log('warn', '处理回复失败:', String(err))
      await finishReaction(messageId, reaction, 'ERROR')
      await replyToMessage(messageId, '❌ 处理失败：' + clip(String(err?.message ?? err), 300))
        .catch(() => undefined)
    }
  }
  ...
  async function replyToMessage(messageId, text) {
    for (const chunk of splitText(text, replyMaxChars)) {
      await apiCall('im.message.reply', () => client.im.v1.message.reply({
