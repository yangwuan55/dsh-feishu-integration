...
export function createQuestionBridge({
  ...
  /** feishuMsgId → batchCtx {rpcId, sessionId, questions, cursor, answers, msgIds} */
  const pendingByFeishuId = new Map()
  /** rpcId → { msgIds:[], answeredByUs } —— 含已结算，防重连重放重复发帖 */
  const seenRpcIds = new Map()

  function clearBatch(batch) {
    for (const msgId of batch.msgIds) pendingByFeishuId.delete(msgId)
  }

  /** 发送第 index 题（1-based）并登记其飞书消息。 */
  async function postRound(batch, index) {
    ...
    let posted = null
    try {
      posted = await postToSession(text, batch.sessionId, parent)
    } catch (err) {
    ...
    if (posted?.messageId) {
      ...
      pendingByFeishuId.set(posted.messageId, batch)
    }
  ...
  async function handleQuestionRequested(rpcId, payload) {
    ...
    const batch = { rpcId, sessionId, questions, cursor: 0, answers: [], msgIds: [] }
    // seen 与 batch 共享同一个 msgIds 数组：resolved 清算与作答清算看到同一份登记
    const seen = { msgIds: batch.msgIds, answeredByUs: false }
