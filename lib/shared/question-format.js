...
/**
 * 把一批提问渲染成飞书文本。多问题批次返回 null（调用方降级为“请到网页端回答”）。
 */
export function formatQuestionText(questions) {
...
export function parseAnswerChoice(text, question) {
  ...
  if (exact !== undefined) {
    if (multi) return { ok: true, selected: [exact] }
    return { ok: true, selected: [exact] }
  }
